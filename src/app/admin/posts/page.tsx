...
export default function AdminDashboard() {
  ...
  const [postForm, setPostForm] = useState({
    ...
    excerpt: "", content: "", imageUrl: "", videoUrl: ""
  });
  ...
  return (
    <div className="admin-page-wrap">
        ...
        <header className="admin-header">
          <div className="ah-left">
            ...
            <div className="admin-tabs">
              <button className={activeTab === 'posts' ? 'active' : ''} onClick={() => setActiveTab('posts')}>Insights & Blog</button>
              ...
              {activeTab === 'posts' ? posts.map(p => (
                <div key={p._id} className="admin-post-item">
                  ...
                  <div className="api-actions">
                    <Link href={`/blog/${p.slug}`} target="_blank" className="api-view">View</Link>
                ...
                <div key={t._id} className="admin-post-item">
                  <div className="api-info">
                    <h3>{t.name}</h3>
                    <div className="api-meta"><span>{t.role} @ {t.company || 'Private'}</span></div>
                  </div>
                  <button onClick={() => handleDelete('testimonials', t._id)} className="api-del">Delete</button>
                </div>
              ))}
            </div>
          </section>
      ...
      <style jsx>{`
        ...
        .api-view { color: var(--P); font-weight: 700; text-decoration: none; }
        ...
        .api-view { color: var(--P); font-weight: 700; font-size: 0.9rem; text-decoration: none; }
      `}</style>
    </div>
  );
}
